...
from diffusers import UNet2DConditionModel, DDPMScheduler, AutoencoderKL, StableDiffusionXLPipeline
...
def cmd_encode(args):
    ...
    vae: AutoencoderKL = AutoencoderKL.from_pretrained(
        VAE_REPO, torch_dtype=torch.float32  # ✅ 改为 float32
    ).to(device)
    vae.enable_slicing()
    vae.enable_tiling()
    ...
    for (path, caption, author) in tqdm(samples):
        ...
        with torch.no_grad(), torch.amp.autocast('cuda', dtype=torch.float16, enabled=(device.type=="cuda")):
            lat = vae.encode(pixel).latent_dist.sample() * VAE_SCALE  # [1,4,128,128]
...
def cmd_decode(args):
    ...
    def decode_one(npz_path: str, out_path: str):
        ...
        lat = torch.from_numpy(z["latent"]).to(device=device, dtype=dtype).unsqueeze(0)  # [1,4,128,128]
        lat = lat / args.scale
...
class LatentCapAuthorDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        lat = z["latent"].astype(np.float16)          # (4,128,128)
...
# -------------------------
# 训练（5 桶 + SDXL 双编码器 + added_cond_kwargs）
# -------------------------
def cmd_train(args):
    ...
    # ====== UNet (SDXL) ======
    print(">> building UNet (SDXL config)…")
    ...
    # ====== 预览图管线（SDXL）======
    dtype = torch.float16 if device.type == "cuda" else torch.float32
    pipe = StableDiffusionXLPipeline.from_pretrained(SDXL_REPO, torch_dtype=dtype).to(device)
    ...
    # ====== 文本编码器（SDXL：两套 tokenizer/text_encoder）======
    tokenizer_1 = CLIPTokenizer.from_pretrained(SDXL_REPO, subfolder="tokenizer")
    tokenizer_2 = CLIPTokenizer.from_pretrained(SDXL_REPO, subfolder="tokenizer_2")

    text_encoder_1 = CLIPTextModel.from_pretrained(
        SDXL_REPO, subfolder="text_encoder",
        torch_dtype=torch.float16 if device.type=="cuda" else torch.float32
    ).to(device)
    text_encoder_1.eval().requires_grad_(False)

    text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(
        SDXL_REPO, subfolder="text_encoder_2",
        torch_dtype=torch.float16 if device.type=="cuda" else torch.float32
    ).to(device)
    text_encoder_2.eval().requires_grad_(False)

    @torch.no_grad()
    def encode_texts_sdxl(text_list: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        # encoder 1
        ids1 = tokenizer_1(text_list, padding="max_length", max_length=MAX_TOKEN,
                           truncation=True, return_tensors="pt").input_ids.to(device)
        # encoder 2
        ids2 = tokenizer_2(text_list, padding="max_length", max_length=MAX_TOKEN,
                           truncation=True, return_tensors="pt").input_ids.to(device)

        with torch.amp.autocast('cuda', enabled=use_amp, dtype=torch.float16):
            out1 = text_encoder_1(ids1, output_hidden_states=True)
            ...
            token_embeds = torch.cat([hidden1, hidden2], dim=-1)  # [B,77,2048]
            pooled = out2.text_embeds                                 # [B,1280]
        return token_embeds, pooled

    # 预计算 uncond
    uncond_tokens, uncond_pooled = encode_texts_sdxl([""])  # [1,77,2048], [1,1280]

    # 文本嵌入 LRU 缓存（缓存 token + pooled）
    CACHE_CAP = getattr(args, "embed_cache_size", 50000)
    cache = OrderedDict()  # str -> (tokens_cpu, pooled_cpu)
    def get_emb_cached_sdxl(s: str):
        if CACHE_CAP <= 0:
            return encode_texts_sdxl([s])  # 禁用缓存
        if s in cache:
            tup = cache.pop(s); cache[s] = tup
        else:
            tok, poo = encode_texts_sdxl([s])
            tup = (tok.detach().cpu(), poo.detach().cpu())
            if len(cache) >= CACHE_CAP:
                cache.popitem(last=False)
            cache[s] = tup
        tokens_cpu, pooled_cpu = cache[s]
        return tokens_cpu.to(device, non_blocking=True), pooled_cpu.to(device, non_blocking=True)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            # batch: lat [B,4,128,128], cap(list[str])，auth(list[str])，preview_text(list[str])
            ...
            for s in chosen_texts:
                if not s:
                    tok_list.append(uncond_tokens.expand(1, -1, -1))
                    poo_list.append(uncond_pooled.expand(1, -1))
                else:
                    tok_i, poo_i = get_emb_cached_sdxl(s)
                    tok_list.append(tok_i)
                    poo_list.append(poo_i)
            token_embeds = torch.cat(tok_list, dim=0)   # [B,77,2048]
            pooled_embeds = torch.cat(poo_list, dim=0)  # [B,1280]

            # —— CFG dropout：token 与 pooled 同步替换成 uncond
            if args.cfg_drop > 0.0:
                m = (torch.rand(B, device=device) < args.cfg_drop).view(B, 1, 1)
                token_embeds = torch.where(m, uncond_tokens.expand(B, -1, -1), token_embeds)
                m2 = m.view(B, 1)
                pooled_embeds = torch.where(m2, uncond_pooled.expand(B, -1), pooled_embeds)

            # —— micro-conditioning: time_ids（统一用目标尺寸与无裁剪）
            H = W = getattr(args, "preview_size", 1024)
            time_ids = torch.tensor([H, W, 0, 0, H, W], device=device, dtype=torch.long).view(1, 6).expand(B, -1)
            ...
            if getattr(args, "preview_every_ckpt", False) and (global_step % args.preview_save_steps == 0 or global_step == 1):
                ...
                # 保存 RAW / EMA 预览
                _save_preview_image_sdxl(pipe, unet, ema, encode_texts_sdxl, uncond_tokens, uncond_pooled, args, ptxt, global_step, device, dtype, use_ema=False)
                _save_preview_image_sdxl(pipe, unet, ema, encode_texts_sdxl, uncond_tokens, uncond_pooled, args, ptxt, global_step, device, dtype, use_ema=True)
...
def _save_preview_image_sdxl(pipe: StableDiffusionXLPipeline, unet, ema: EMA,
                             encode_texts_sdxl, uncond_tokens, uncond_pooled,
                             args, prompt_text: str, step: int, device, dtype, use_ema: bool):
    if not prompt_text:
        return
    ...
    # 2) 文本嵌入
    pos_tok, pos_poo = encode_texts_sdxl([prompt_text])
    neg_text = getattr(args, "preview_negative", "")
    if neg_text:
        neg_tok, neg_poo = encode_texts_sdxl([neg_text])
    else:
        neg_tok, neg_poo = uncond_tokens, uncond_pooled

    H = W = args.preview_size
    g = torch.Generator(device=device.type)
    if args.preview_seed is not None:
        g = g.manual_seed(args.preview_seed)

    image = pipe(
        prompt_embeds=pos_tok,
        negative_prompt_embeds=neg_tok,
        pooled_prompt_embeds=pos_poo,
        negative_pooled_prompt_embeds=neg_poo,
        num_inference_steps=args.preview_steps,
        guidance_scale=args.preview_scale,
        width=W, height=H,
        generator=g,
        original_size=(H, W), target_size=(H, W), crops_coords_top_left=(0, 0),
    ).images[0]
...
def save_ckpt(args, unet, ema: EMA, step: int, prediction_type: str):
    ...
    with open(os.path.join(ema_dir, "config.json"), "w", encoding="utf-8") as f:
        json.dump(ema_model.config, f, indent=2)
...
def cmd_infer(args):
    ...
    g = torch.Generator(device=device.type)
    g = g.manual_seed(args.seed) if args.seed is not None else g.manual_seed(torch.seed())
...
# -------------------------
def build_parser():
    p = argparse.ArgumentParser("SDXL UNet Train & Infer (5-way conditioning, runtime dual-text-encoder)")
    ...
    # train
    pt = sub.add_parser("train", help="训练 SDXL UNet（5 桶采样 + 双文本编码器 + CFG dropout）")
    ...
    pt.add_argument("--preview_size", type=int, default=1024, help="预览图边长(正方形)")
    pt.add_argument("--preview_negative", type=str, default="", help="预览图的负面提示词")

    pt.set_defaults(func=cmd_train)

    # infer
    pi = sub.add_parser("infer", help="替换管线 UNet 推理出图（SDXL）")
    ...
    # decode
    pd = sub.add_parser("decode", help="将 latent(.npz) 解码为图片（使用 SDXL 的 VAE）")
    ...
    pd.add_argument("--scale", type=float, default=VAE_SCALE, help="latent 缩放（SDXL=0.13025）")
...
def main():
    ...
    if not hasattr(args, "func"):
        print("Use one of subcommands: encode | train | infer | decode. Example:\n"
              "  python sdxl_unet_runtime_text.py encode --csv data.csv --out_dir ./latent_db\n"
              "  python sdxl_unet_runtime_text.py train  --data_dir ./latent_db --out_dir ./ckpts --vpred --min_snr_gamma 5 --preview_every_ckpt\n"
              "  python sdxl_unet_runtime_text.py infer  --unet_path ./ckpts/step_20000_ema/unet_ema.safetensors "
              "--prompt 'best quality, 1girl, miko' --vpred --out sample.png\n"
              "  python sdxl_unet_runtime_text.py decode --input ./latent_db --out_dir ./decoded")
        return
    args.func(args)
