...
csv_file_path = "/mnt/shared/data/cglist_250714.csv"
    ...
    while True:
        ...
        for i in range(len(items)):
            ...
            if match:
                ...
                if csv_reader.has_value_in_column_index(4, game_id):
                    # print(f"[Exists] Game ID found: {game_id}")
                    continue
                else:
                    print(f"[New] Game ID found: {game_id}, Title: {title}, URL: {link}")
                    log.write(f"[NEW] Game ID found: {game_id}, Title: {title}, URL: {link}\n")
                    log.flush()
            else:
