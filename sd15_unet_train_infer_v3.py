...
from transformers import CLIPTokenizer, CLIPTextModel, Blip2Processor, Blip2ForConditionalGeneration
...
def _build_variants_from_cap_author(caption_tags: str, caption_nl: str, author: str):
    """
    基于原始 caption_tags/caption_nl/author 构造变体文本，并生成可用 mask（去重后）。
    返回 texts(list[str]), mask(np.bool_), preview_text(str)
    """
...
# - caption_nl:   用 BLIP2 批量生成的自然语言描述
# - author:       原 CSV 的作者字段
# 支持 --blip_batch 批量推理
# -------------------------
def cmd_encode(args):
    ...
    def flush_batch():
        ...
        for (path, caption_tags, author), cap in zip(metas_batch, captions):
            ...
            #     lat = vae.encode(pixel).latent_dist.sample() * VAE_SCALE
            base = sha1(path) + ".npz"
...
class LatentCapAuthorDataset(Dataset):
        ...
        with open(index_jsonl, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    j = json.loads(line)
                    fname = j["npz"]
                    cap_tags = j.get("caption_tags", "")
                    cap_nl = j.get("caption_nl", "")
                    auth = j.get("author", "")
                    full_path = os.path.join(root_dir, fname)
                    if not os.path.isfile(full_path):
                        continue
                    # 为了 DataLoader 预览，预先计算一次 preview_text
                    _, _, preview = _build_variants_from_cap_author(cap_tags, cap_nl, auth)
                    self.items.append((fname, cap_tags, cap_nl, auth, preview))
                except Exception:
    ...
    def __getitem__(self, i):
        fname, cap_tags, cap_nl, auth, preview = self.items[i]
        z = np.load(os.path.join(self.root, fname), allow_pickle=False)
        lat = z["latent"].astype(np.float16)          # (4,64,64)
        return torch.from_numpy(lat), cap_tags, cap_nl, auth, preview
...
def cmd_train(args):
    ...
    def sample_variant_index(mask_i: np.ndarray) -> int:
        p = PROBS * mask_i.astype(np.float64)
        s = p.sum()
        if s <= 0:
            # 该样本完全无文本 -> 返回任意索引，但稍后会走 uncond
            return int(np.random.choice(len(VARIANT_NAMES)))
        p = p / s
        return int(np.random.choice(len(VARIANT_NAMES), p=p))
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            lat, cap_tags, cap_nls, auths, preview_texts = batch
