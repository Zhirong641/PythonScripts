...
def _artist_phrase(artists: List[str]) -> str:
    if not artists:
        return ""
    a = _normalize(artists[0])
    if len(artists) > 1 and random.random() < 0.25:
        b = _normalize(artists[1])
        return f"by {a} and {b}"
    return f"by {a}"
...
# 说明：Camie 的 meta/rating/year 等类别用于分析很有用，但**不适合作为生成提示**；
# 另外，站点/导出相关标签（例如 sprite、tachi-e、official_art、watermark、logo、signature、game cg）应剔除。:contentReference[oaicite:2]{index=2}
_BLACKLIST = {
    # 背景/导出
...
def generate_phrase_variants(
    ...
    max_chars: int = 1,  # 新增：每条最多保留多少角色名
) -> List[str]:
    # 预清洗 general
    g0 = []
    ...
    for _ in range(max(k, 1)):
        ...
        # 若角色+作者已超预算，尝试只留角色；再不行只留作者；仍不行则空
        current = ", ".join(parts) if parts else ""
        if _tok_len(current) > token_budget:
            if char_phrase and artist and _tok_len(char_phrase) <= token_budget:
                current = char_phrase
            elif artist and _tok_len(artist) <= token_budget:
                current = artist
            else:
                current = ""  # 极端情况：后续只靠 general 填到预算内
...
def generate_variants_with_nl_list(
    general_tags: List[str],
    artists: List[str],
    nl_texts: List[str],             # 你已有的自然语言描述列表（英文为宜）
    k: int = 6,
    token_budget: int = 70,
    phrase_ratio: float = 0.8,       # 约 80% 短语式 + 20% 自然语言
    dropout: float = 0.20,
    max_general_per_variant: int = 10,
    head_keep: int = 24,
    characters: Optional[List[str]] = None,
    ...
    phrase_caps = generate_phrase_variants(
        ...
        dropout=dropout, max_general_per_variant=max_general_per_variant, head_keep=head_keep, characters=characters
    )
    ...
    if n_nl > 0 and nl_texts:
        ...
        for txt in pool:
            ...
            if art:
                s = f"{art}, {s}" if random.random() < 0.5 else f"{s}, {art}"
...
if __name__ == "__main__":
    ...
    caps = generate_variants_with_nl_list(
        general, artists, nl_list, k=20, phrase_ratio=1.00, token_budget=70, max_general_per_variant=20, characters=characters
    )
